...
oFilterBox = function () {
	...
	this.draw = function (gr, x, y) {
		var bx = x;
		var by = y;
		var bw = this.inputbox.w + DPI.scale(44);
		...
		for (var i = 0; i < cFilterBox.h - 2; i += 2) {
			gr.FillSolidRect(bx + DPI.scale(22) + cFilterBox.w, by + 2 + i, 1, 1, RGB(100, 100, 100));
		};
		this.inputbox.draw(gr, bx + DPI.scale(22), by, 0, 0);
	};
...
oBrowser = function (name) {
	...
	this.draw = function (gr) {
		...
		if (ppt.showHeaderBar) {
			...
			var tx = cFilterBox.x + cFilterBox.w + DPI.scale(22) + 5;
...
class ZoomUpdater {
	constructor(ppt) {
		this._zoom;
		this._withoutExtraZoom;
		const defaultFontSize = function () {
			let default_font;
			...
			if (g_instancetype === 0) {
				...
				// some other UI componnets that I don't know yet.
			}
			try {
		...
		this.updateZoom = function () {
			this._withoutExtraZoom = (defaultFontSize() / 12 * 100) >> 0;
			this._zoom = this._withoutExtraZoom + ppt.extra_font_size;
		}
	}
	scale(value) {
		return Math.round(value * this._zoom / 100);
	}
	scaleWithoutExtra(value) {
		return Math.round(value * this._withoutExtraZoom / 100);
	}
	updateZoom() { };
}

const DPI = new ZoomUpdater(ppt);
...
export function on_mouse_lbtn_down(x, y) {
	g_lbtn_click = true;
...
export function on_mouse_lbtn_up(x, y) {
...
export function on_mouse_lbtn_dblclk(x, y, mask) {
	if (y >= brw.y) {
...
export function on_mouse_rbtn_up(x, y) {
	// inputBox
	if (ppt.showHeaderBar && ppt.showFilterBox && g_filterbox.inputbox.visible) {
...
export function on_mouse_wheel(step) {
...
function get_metrics() {
	...
	cPlaylistManager.topbarHeight = DPI.scale(cPlaylistManager.default_topbarHeight);
	cPlaylistManager.botbarHeight = DPI.scale(cPlaylistManager.default_botbarHeight);
	cPlaylistManager.rowHeight = DPI.scale(cPlaylistManager.default_rowHeight);
	cPlaylistManager.scrollbarWidth = DPI.scale(cPlaylistManager.default_scrollbarWidth);

	if (ppt.showHeaderBar) {
		ppt.headerBarHeight = DPI.scale(ppt.defaultHeaderBarHeight);
	...
	ppt.rowHeight = DPI.scale(_defaultRowHeight);
	// TODO: scrollbarWidth do not change with font change. but should change with ^+mouse step.
	//cScrollBar.width = Math.floor(cScrollBar.defaultWidth * g_zoom_percent / 100);
	cScrollBar.minCursorHeight = DPI.scale(cScrollBar.defaultMinCursorHeight);
	//
	cover.margin = DPI.scale(cover.default_margin);
	...
	cFilterBox.w = DPI.scale(cFilterBox.default_w);
	cFilterBox.h = DPI.scale(cFilterBox.default_h);
...
export function on_font_changed() {
	get_font();
	get_metrics();
	brw.repaint();
};

export function on_colours_changed() {
	get_colors();
	get_images();
	if (brw)
...
export function on_script_unload() {
	brw.g_time && window.ClearInterval(brw.g_time);
	brw.g_time = false;
};

//=================================================// Keyboard Callbacks
export function on_key_up(vkey) {
...
export function on_key_down(vkey) {
...
//=================================================// Playback Callbacks
export function on_playback_stop(reason) {
	g_seconds = 0;
...
export function on_playback_new_track(metadb) {
	g_metadb = metadb;
	g_radio_title = "loading live tag ...";
	g_radio_artist = "";
...
export function on_playback_starting(cmd, is_paused) { };

export function on_playback_time(time) {
	g_seconds = time;
	g_time_remaining = ppt.tf_time_remaining.Eval(true);
...
//=================================================// Playlist Callbacks
export function on_playlists_changed() {
...
export function on_playlist_switch() {
...
export function on_playlist_items_added(playlist_idx) {
...
export function on_playlist_items_removed(playlist_idx, new_count) {
...
export function on_playlist_items_reordered(playlist_idx) {
	if (playlist_idx == g_active_playlist) {
		g_focus_id = getFocusId(g_active_playlist);
		brw.populate(true);
	};
...
export function on_item_focus_change(playlist, from, to) {
...
export function on_metadb_changed(handles) {
	if (!brw.list)
		return;
...
export function on_item_selection_change() {
	brw.repaint();
};

export function on_playlist_items_selection_change() {
	brw.repaint();
};

export function on_focus(is_focused) {
	if (is_focused) {
		plman.SetActivePlaylistContext();
		g_selHolder.SetPlaylistSelectionTracking();
	} else {
...
export function on_notify_data(name, info) {
	switch (name) {
...
export function on_drag_enter() {

}

export function on_drag_leave() {

}

export function on_drag_over(action, x, y, mask) {
...
export function on_drag_drop(action, x, y, mask) {
	if (y < brw.y || (plman.ActivePlaylist > -1 && plman.IsPlaylistLocked(plman.Activeplaylist))) {
		action.Effect = 0;
	} else {
