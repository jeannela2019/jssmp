/**
 * This type file should be compatible with foo_spider_monkey_panel.dll v1.6.0,
 ...
 * @return {number}
 */
// declare function setInterval(func: TimerHandler, delay: number, ...func_args: any[]): number;
 ...
 */
// declare function setTimeout(func: TimerHandler, delay: number, ...func_args: any[]): number;
...
//      * ```
//      */
//     // ActiveX_CreateArray(arr:Array<T>, element_variant_type: number):

//     // ActiveX_Get(prop_name: string|number):any;

//     // ActiveX_Set(prop_name: string|number): any;
// }
...
interface IGdiUtils {
	 ...
	 */
	Font(name: string, size_px: number, style?: number): IGdiFont;
...
interface IFbPlaylistManager {
	 ...
	 */
	GetPlaylistLockedActions(playlistIndex: number): Array<string>;
	 ...
	 *
	 * @param {number} playlistIndex_removed
	 * @return {boolean}
	 */
	RemovePlaylistSwitch(playlistIndex_removed: number): boolean;
	 ...
	 *   - 'ExecuteDefaultAction'
	*/
	SetPlaylistLockedActions(playlistIndex: number, lockedActions: Array<string>): void;
...
interface Window {
	...
	*/
	 ...
	 * @readonly
	 */
	readonly JsMemoryStats: JsMemoryStatsObject,
...
interface IFbFileInfo {
	 ...
	 * @return {string}
	 */
	MetaValue(idx: number, value_index: number): string;
...
interface JsPackageDirs {
	...
	/**
	 * Persistent and unique directory inside foobar2000 profile folder
	 * that can be used to store runtime data (e.g. cache).
	 */
	Storage: string;
}
