 ...
 * @return {number}
 */
declare function setInterval(func, delay: number, ...func_args: any[]): number;
 ...
 */
declare function setTimeout(func, delay: number, ...func_args: any[]): number
...
interface IFbUtils {
   ...
   */
  DoDragDrop(window_id, handle_list, effect, options): number;
...
interface IUtils {
   ...
   */
  GetAlbumArtEmbedded(rawpath, art_id): GdiBitmap;
   ...
   */
  GetAlbumArtV2(handle, art_id, need_stub): GdiBitmap;
   ...
   * @return {string}
   */
  GetPackagePath(package_id): string
   ...
   */
  Glob(pattern, exc_mask, inc_mask): string[]
   ...
   */
  ReadTextFile(filename, codepage): string
   ...
   */
  ReadINI(filename, section, key, default_val): string
   ...
   */
  ShowHtmlDialog(window_id, code_or_path, options): any
...
interface Window {
   ...
   * @return {FbTooltip}
   */
  CreateTooltip(font_name, font_size_px, font_style): FbTooltip;
   ...
   * @param {boolean=} [force=false] If true, will repaint immediately, otherwise a repaint task will be *scheduled*.
   */
  RepaintRect(x, y, w, h, force): void
   ...
   * @param {*=} val
   */
  SetProperty(name: string, val): void;
...
interface FbPlaylistRecycler {
   ...
   * @return {FbMetadbHandleList}
   */
  GetContent(index): FbMetadbHandleList;
...
interface FbProfiler {
   ...
   */
  Print(additionalMsg, printComponentInfo): void
}
...
interface FbTitleFormat {
   ...
   */
  EvalWithMetadbs(handle_list): string[]
}
...
interface FbTooltip {
   ...
   * @param {number=} [font_style=0] See Flags.js > FontStyle
   */
  SetFont(font_name, font_size_px, font_style): void;
...
interface GdiBitmap {
   ...
   * @return {Array<number>}
   */
  GetColourScheme(max_count): number[]
...
interface GdiGraphics {
   ...
   */
  DrawString(str, font, colour, x, y, w, h, flags): void
   ...
   */
  EstimateLineWrap(str, font, max_width): Array<Array<string | number>>
   ...
   * @param {number=} [alpha=255] Valid values 0-255.
   */
  GdiAlphaBlend(img, dstX, dstY, dstW, dstH, srcX, srcY, srcW, srcH, alpha): void
   ...
   * @param {number} srcH
   */
  GdiDrawBitmap(img, dstX, dstY, dstW, dstH, srcX, srcY, srcW, srcH): void
   ...
   */
  GdiDrawText(str, font, colour, x, y, w, h, format): void
   ...
   */
  MeasureString(str, font, x, y, w, h, flags): MeasureStringInfo;
...
interface ThemeManager {
   ...
   */
  DrawThemeBackground(gr, x: number, y: number, w: number, h: number, clip_x?: number, clip_y?: number, clip_w?: number, clip_h?: number): void // (void) [, clip_x][, clip_y][, clip_w][, clip_h]
